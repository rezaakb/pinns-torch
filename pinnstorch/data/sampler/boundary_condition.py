...
class DirichletBoundaryCondition(SamplerBase):
    def __init__(self,
                 ...
                 discrete: bool = False):
        """
        Initialize a mesh sampler for collecting training data in upper and lower boundaries
        ...
        spatial_upper_bound, time_upper_bound, solution_upper_bound = mesh.on_upper_boundary(self.solution_names)
        spatial_lower_bound, time_lower_bound, solution_lower_bound = mesh.on_lower_boundary(self.solution_names)
        ...
        for solution_name in self.solution_names:
            solution_bound[solution_name] = np.vstack([solution_upper_bound[solution_name],
        ...
        (self.spatial_domain_sampled,
         ...
         self.solution_sampled) = self.sample_mesh(num_sample, (spatial_bound, time_bound, solution_bound))
    ...
    def sample_mesh(self, num_sample, flatten_mesh):
        ...
        If idx_t is defined, only points on that time will be selected.
        If num_sample is not defined the whole points will be selected.
        ...
        if self.discrete:
            ...
            flatten_mesh = [np.vstack((flatten_mesh_[self.idx_t:self.idx_t+1, :],
...
class PeriodicBoundaryCondition(SamplerBase):
    def __init__(self,
        ...
        (self.spatial_domain_sampled,
         self.time_domain_sampled) = self.sample_mesh(num_sample,
                                                     (spatial_upper_bound,
                                                     time_upper_bound,
                                                     spatial_lower_bound,
                                                     time_lower_bound))
    ...
    def sample_mesh(self, num_sample, flatten_mesh):
        ...
        if self.discrete:
            flatten_mesh = [flatten_mesh_[self.idx_t:self.idx_t+1, :] for flatten_mesh_ in flatten_mesh]
        
        if num_sample is None:
            return self.convert_to_tensor((np.vstack((flatten_mesh[0],flatten_mesh[2])),
                                          np.vstack((flatten_mesh[1],flatten_mesh[3]))))
        else:
            idx = np.random.choice(range(flatten_mesh[0].shape[0]), num_sample, replace=False)
            return self.convert_to_tensor((np.vstack((flatten_mesh[0][idx,:],flatten_mesh[2][idx,:])),
