...
class MeshBase:
    ...
    def domain_bounds(self):
        ...
        """
        mesh = np.hstack((self.spatial_domain_mesh.reshape(-1, self.spatial_dim),
    ...
    def on_lower_boundary(self, solution_names):
        """
        ...
        """
        spatial_domain = np.ones((self.time_domain_mesh.shape[1],
                                  self.spatial_dim)) * self.lb[0:-1]        
        time_domain = self.time_domain_mesh[0, :]
        solution_domain = {solution_name: self.solution[solution_name][0, :][:, None]
                           for solution_name in solution_names}
        
        return spatial_domain, time_domain, solution_domain

    def on_upper_boundary(self, solution_names):
        """
        ...
        """
        spatial_domain = np.ones((self.time_domain_mesh.shape[1],
                                  self.spatial_dim)) * self.ub[0:-1]    
        time_domain = self.time_domain_mesh[-1, :]
        solution_domain = {solution_name: self.solution[solution_name][-1, :][:, None]
                           for solution_name in solution_names}
        return spatial_domain, time_domain, solution_domain

    def on_initial_boundary(self, solution_names, idx=0):
        """
        ...
        solution_domain = {solution_name: self.solution[solution_name][:, idx:idx+1]
                           for solution_name in solution_names}
        
        return spatial_domain, time_domain, solution_domain


    def collection_points(self, N_f):
        """
        ...
        return f[:,0:self.spatial_dim], f[:,self.spatial_dim:self.spatial_dim+1]


    def flatten_mesh(self, solution_names):
        """
...
class Mesh(MeshBase):
    def __init__(self,
        ...
        :param spatial_domain: Instance of a spatial domain class.
        :param time_domain: Instance of a time domain class.
...
class PointCloud(MeshBase):
    def __init__(self,
        ...
        self.spatial_domain, self.time_domain, self.solution = data.spatial, data.time, data.solution
        ...
        # Generate Mesh for both spatial and time domain
        self.spatial_domain_mesh = np.zeros((spatial_num_points, time_num_points, self.spatial_dim))
        
        for i, interval in enumerate(self.spatial_domain):
            self.spatial_domain_mesh[:, :, i] = np.tile(interval, (1, time_num_points))
                
        self.time_domain_mesh  = np.tile(self.time_domain, (1, spatial_num_points)).T
