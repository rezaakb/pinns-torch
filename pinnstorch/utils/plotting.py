...
def plot_kdv(mesh, preds, train_datasets, val_dataset, file_name):
    ...
    h = ax.imshow(exact_u, interpolation='nearest', cmap='rainbow', 
                  extent=[mesh.time_domain[:].min(), mesh.time_domain[:].max(),
                          mesh.spatial_domain[:].min(), mesh.spatial_domain[:].max()], 
                  origin='lower', aspect='auto')
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(h, cax=cax)
...
def plot_ac(mesh, preds, train_datasets, val_dataset, file_name):
    ...
    h = ax.imshow(exact_u, interpolation='nearest', cmap='seismic', 
                  extent=[mesh.time_domain[:].min(), mesh.time_domain[:].max(),
                          mesh.spatial_domain[:].min(), mesh.spatial_domain[:].max()], 
                  origin='lower', aspect='auto')
    divider = make_axes_locatable(ax)
...
def plot_schrodinger(mesh, preds, train_datasets, val_dataset, file_name):
    ...
    line = np.linspace(mesh.spatial_domain[:].min(),
                       mesh.spatial_domain[:].max(), 2)[:,None]
...
def plot_burgers_continuous_forward(mesh, preds, train_datasets, val_dataset, file_name):
    ...
    X_u_train = np.vstack([x_i[0],
                           x_b[0]])
    
    X_u_time = np.vstack([t_i,
                          t_b])

    
    X_u_train = np.hstack([X_u_train,
                           X_u_time])
