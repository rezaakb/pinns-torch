...
class RungeKutta(torch.nn.Module):
    def __init__(self,
                 ...
                 q: int = None):
        """
        Initialize a RungeKutta object for solving differential equations using Implicit Runge-Kutta methods.
        ...
        :param q: Order of the Implicit Runge-Kutta method. If not provided, it is automatically calculated.
        """
        super().__init__()
    ...
    def load_irk_weights(self,
                         root_dir,
                         q: int) -> None:
        """
        Load the weights and coefficients for the Implicit Runge-Kutta method and save in the dictionary.
    ...
    def forward(self, outputs, mode, solution_names, collection_points_names):
        ...
        for solution_name, collection_points_name in zip(solution_names, collection_points_names):
            ...
            if mode == 'inverse_discrete_1':
                outputs[solution_name] = outputs[solution_name] - \
                                         self.dt * torch.matmul(outputs[collection_points_name], self.alpha)
            
            elif mode == 'inverse_discrete_2':
                outputs[solution_name] = outputs[solution_name] + \
                                         self.dt * torch.matmul(outputs[collection_points_name], (self.beta - self.alpha))
            
            elif mode == 'forward_discrete':
                outputs[solution_name] = outputs[solution_name] - \
                                         self.dt * torch.matmul(outputs[collection_points_name], self.weights)
