...
from pinnstorch.utils.module_fn import sse, mse, relative_l2_error, fix_extra_variables, requires_grad
...
class PINNModule(LightningModule):
    ...
    def __init__(
        ...
        :param automatic_optimization: A boolean that control automatic_optimization.
        """
        super().__init__()
        ...
        self.functions = {'runge_kutta': self.rk,
    ...
    def model_step(
        ...
        for key, value in batch.items(): 
            loss, preds = key(value, loss, **self.functions)
        return loss, preds

    def capture_graph(self, batch):
        """Fills the graph's input memory with new data to compute on.
        If the batch_size is not specified, the model uses all the available data, and there is no need to copy the data.
    ...
    def copy_batch(self, batch) -> None:
        ...
        for key in batch:
            ...
            x = [spatial_.requires_grad_(False).copy_(spatial_new[i]) \
                 for i, spatial_ in enumerate(spatial)]
            if solution_new is not None:
                solution = {key_sol: solution[key_sol].copy_(solution_new[key_sol])
    ...
    def training_step(
        ...
        self.g.replay() includes forward, backward, and step.
    ...
    def eval_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> None:
        ...
        with torch.set_grad_enabled(True):
            ...
            if self.rk:
                error_dict = {solution_name: relative_l2_error(preds[solution_name][:,-1:], u[solution_name]) \
                              for solution_name in solution}
            else:
                error_dict = {solution_name: relative_l2_error(preds[solution_name], u[solution_name]) \
                              for solution_name in solution}
