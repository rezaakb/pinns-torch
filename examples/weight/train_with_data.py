...
def read_data_fn(root_path):
    ...
    exact_body_x = np.real(data["truck_data"][:, 0]).astype("float32")[None, :]            # car z-axis displacement
    exact_body_y = np.real(data["truck_data"][:, 1]).astype("float32")[None, :]            # car z-axis displacement
    ...
    exact_body_z = -np.real(data["truck_data"][:, 2]).astype("float32")[None, :]            # car z-axis displacement
    exact_body_roll = np.real(data["truck_data"][:, 3]).astype("float32")[None, :]    # car pitch
    exact_body_pitch = -np.real(data["truck_data"][:, 4]).astype("float32")[None, :]    # car pitch
    exact_tire_fl_z = np.real(data["truck_data"][:, 6]).astype("float32")[None, :]      # front left tire z-axis displacement
    exact_tire_fr_z = np.real(data["truck_data"][:, 7]).astype("float32")[None, :]      # front left tire z-axis displacement
    exact_tire_rl_z = np.real(data["truck_data"][:, 8]).astype("float32")[None, :]      # rear left tire z-axis displacement
    exact_tire_rr_z = np.real(data["truck_data"][:, 9]).astype("float32")[None, :]      # rear left tire z-axis displacement
    
    data_input = np.concatenate((exact_body_vx, 
                                exact_body_vy, 
                                exact_body_z, 
                                exact_body_roll,
                                exact_body_pitch,
                                ...
                                exact_tire_fr_z,
                                exact_tire_rl_z,
                                exact_tire_rr_z), axis=1)

    return pinnstorch.data.TimeSeriesDomain(data_input=data_input, time_interval=dt)
